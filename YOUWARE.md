...
### v10.0-erp-posto-1.4 (2026-06-17)
- **Status:** ✅ Migração para novo projeto Supabase "erp-posto 1.4"
- **Descrição:** Configuração completa do banco de dados no projeto Supabase "erp-posto 1.4" (sa-east-1, PostgreSQL 17). Todas as tabelas essenciais criadas com RLS permissivo.
- **Projeto Supabase:** erp-posto 1.4 (ID: qmafdxxwmusdmqoajide)
- **URL:** https://qmafdxxwmusdmqoajide.supabase.co
- **Região:** sa-east-1 (São Paulo)
- **Banco:** PostgreSQL 17.6.1.127
- **Tabelas criadas (4 essenciais):**
  - `module_data` — Persistência genérica JSONB por módulo/período (tabela principal)
  - `usuarios` — Usuários do sistema
  - `parametros_sistema` — Dados cadastrais do posto
  - `arquivos` — Central global de metadados de arquivos
- **RLS:** 16 políticas permissivas (CRUD anônimo liberado para desenvolvimento)
- **Dados iniciais:** Registro de configurações da empresa inserido
- **Build:** ✅ Sucesso (index-36StkRbA.js, 1.276 kB)
- **Arquivos modificados:**
  - `.env` — Credenciais atualizadas para erp-posto 1.4
- **GitHub:** https://github.com/gerentepostocargapesada-hub/erp-1.4 (branch: main)

### v9.0-supabase-complete (2026-06-17)
- **Status:** ✅ Migração Completa localStorage → Supabase
- **Descrição:** Migração definitiva de todos os 8 módulos do FuelOps Pro do localStorage para Supabase PostgreSQL. Persistência via tabela `module_data` (JSONB). Módulo Regulamentação agora salva exclusivamente no Supabase (sem fallback localStorage), incluindo upload de arquivos reais para Supabase Storage (bucket "documentos").
- **Arquivos criados:**
  - `src/services/supabasePersistence.ts` — Camada de persistência abstrata com load/save genéricos, upload de arquivos para Supabase Storage, função `uploadDocumento()` dedicada

...
async function loadFromStorage(): Promise<any> {
  try {
    const { data } = await supabase
      .from('module_data')
      .select('data')
      .eq('module_name', 'configuracoes')
      .eq('period_key', 'all')
      .single();
...
async function await saveToStorage(data: any): Promise<void> {
  try {
    await supabase.from('module_data').upsert({
      module_name: 'configuracoes',
      period_key: 'all',
      data,
      updated_at: new Date().toISOString(),
    }, { onConflict: 'module_name,period_key' });
  } catch (e) { console.error('[Supabase] save configuracoes:', e); }
}

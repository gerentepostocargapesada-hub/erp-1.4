...
async function loadFromStorage(): Promise<Record<string, any>> {
  try {
    const { data } = await supabase
      .from('module_data')
      .select('data')
      .eq('module_name', 'lubrificacao')
      .eq('period_key', 'all')
      .single();
    return data?.data ?? {};
  } catch { return {}; }
...
async function await saveToStorage(data: Record<string, any>): Promise<void> {
  try {
    await supabase.from('module_data').upsert({
      module_name: 'lubrificacao',
      period_key: 'all',
      data,
      updated_at: new Date().toISOString(),
    }, { onConflict: 'module_name,period_key' });
  } catch (e) { console.error('[Supabase] save lubrificacao:', e); }
}
...
export default function TrocaOleoLubrificacao() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (saved && Object.keys(saved).length > 0) {
        allDataRef.current = saved;
        const keys = Object.keys(saved).sort().reverse();
        if (keys.length > 0) loadPeriod(keys[0]);
      }
    })();
  ...
  useEffect(() => {
    ...
    await saveToStorage(allDataRef.current);

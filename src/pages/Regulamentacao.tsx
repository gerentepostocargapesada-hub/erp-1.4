...
async function carregarDados(): Promise<Record<string, Documento[]>> {
  try {
    const { data } = await supabase
      .from('module_data')
      .select('data')
      .eq('module_name', 'regulamentacao')
      .eq('period_key', 'all')
      .single();
    return data?.data ?? {};
  } catch { return {}; }
...
async function salvarDados(dados: Record<string, Documento[]>): Promise<void> {
  try {
    await supabase.from('module_data').upsert({
      module_name: 'regulamentacao',
      period_key: 'all',
      data: dados,
      updated_at: new Date().toISOString(),
    }, { onConflict: 'module_name,period_key' });
  } catch (e) { console.error('[Supabase] save regulamentacao:', e); }
}
...
export default function Regulamentacao() {
  ...
  useEffect(() => {
    void (async () => {
      const saved = await carregarDados();
      if (saved && Object.keys(saved).length > 0) setDados(saved);
    })();
  ...
  // ── Persistência ─────────────────────────────────────
  useEffect(() => {
    void (async () => { await salvarDados(dados); })();
  }, [dados]);

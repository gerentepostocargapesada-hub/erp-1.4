...
import { loadAllModuleData, saveAllModuleData, uploadDocumento, MODULE_NAMES } from "../services/supabasePersistence";
...
/* Dados salvos exclusivamente no Supabase — sem localStorage */
...
export default function Regulamentacao() {
  ...
  const handleAdicionarDocumento = useCallback(async () => {
    ...
    if (formArquivo) {
      try {
        const url = await uploadDocumento(formArquivo, drawerAberto, docId);
        arquivoUrl = url ?? undefined;
      } catch (err) {
        console.error("[Regulamentação] Erro ao enviar arquivo para Supabase Storage:", err);
        alert("Erro ao enviar arquivo para o Supabase Storage. Verifique sua conexão e tente novamente.");
        setUploading(false);
        return;
      }
    }
      ...
      historico: [
        {
          ...
          data,
          hora,
          acao: "Documento criado e arquivo enviado ao Supabase Storage",
          usuario: "Usuário Atual",
        },
      ],
  ...
  const handleSubstituirArquivo = useCallback(
    async (orgaoId: string, docId: string, arquivo: File) => {
      setUploading(true);
      const { data, hora } = fmtDataHora();
      ...
      let arquivoUrl: string | undefined;
      try {
        const url = await uploadDocumento(arquivo, orgaoId, docId);
        arquivoUrl = url ?? undefined;
      } catch (err) {
        console.error("[Regulamentação] Erro ao enviar arquivo substituto:", err);
        alert("Erro ao enviar arquivo para o Supabase Storage. Verifique sua conexão e tente novamente.");
        setUploading(false);
        return;
      }
      ...
      setDados((prev) => ({
        ...
        [orgaoId]: (prev[orgaoId] || []).map((doc) => {
          ...
          const novoHistorico: DocumentoHistorico = {
            ...
            data,
            hora,
            acao: `Arquivo substituído por Usuário Atual (versão ${novaVersao.versao}) — salvo no Supabase Storage`,
            usuario: "Usuário Atual",
          };
          return {
            ...doc,
            ...
            >
              ...
              <div className="px-6 py-4 border-t border-[#262a31] flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  disabled={!formNome.trim() || uploading}

...
-- ############################################################################
-- ############################################################################
--   TABELA GENÉRICA: MODULE_DATA (Persistência JSONB por módulo)
--   Usada por supabasePersistence.ts para salvar dados de todos os módulos.
-- ############################################################################
-- ############################################################################
...
CREATE TABLE IF NOT EXISTS module_data (
    ...
    updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE(module_name, period_key)
);

COMMENT ON TABLE  module_data IS 'Persistência genérica JSONB por módulo e período. Substitui localStorage.';
COMMENT ON COLUMN module_data.module_name IS 'Nome do módulo: regulamentacao, manutencao, financeiro, estrategia, atendimento, configuracoes, lubrificacao, ponto';
COMMENT ON COLUMN module_data.period_key IS 'Chave do período (ex: 2026-06) ou identificador genérico';

CREATE INDEX IF NOT EXISTS idx_module_data_module ON module_data (module_name);
CREATE INDEX IF NOT EXISTS idx_module_data_period ON module_data (module_name, period_key);

-- Habilitar RLS (Row Level Security)
ALTER TABLE module_data ENABLE ROW LEVEL SECURITY;

-- Política permissiva para acesso anônimo (para desenvolvimento)
-- IMPORTANTE: Em produção, restringir para auth.role() = 'authenticated'
CREATE POLICY "anon_access_module_data" ON module_data
    FOR ALL USING (true) WITH CHECK (true);


-- ############################################################################
-- STORAGE: Criar Bucket para Documentos
-- NOTA: Buckets são criados via Dashboard ou via SQL:
-- ############################################################################

-- O bucket "documentos" deve ser criado no Dashboard do Supabase:
--   Storage → New Bucket → Nome: "documentos" → Público: Sim
-- Ou via SQL (caso a extensão storage esteja disponível):
INSERT INTO storage.buckets (id, name, public)
VALUES ('documentos', 'documentos', true)
ON CONFLICT (id) DO NOTHING;

-- Política para permitir uploads anônimos (desenvolvimento)
-- IMPORTANTE: Em produção, restringir para usuários autenticados
CREATE POLICY "allow_public_upload_documentos"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'documentos');

CREATE POLICY "allow_public_read_documentos"
ON storage.objects FOR SELECT
USING (bucket_id = 'documentos');

CREATE POLICY "allow_public_update_documentos"
ON storage.objects FOR UPDATE
USING (bucket_id = 'documentos');

CREATE POLICY "allow_public_delete_documentos"
